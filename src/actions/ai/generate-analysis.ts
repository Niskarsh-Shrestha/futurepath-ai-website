...
/**
 * Generates (or returns the existing cached) AI analysis for a submitted
 * assessment. Per the spec's caching requirement, this NEVER regenerates
 * if an analysis already exists — that's exclusively what
 * regenerate-analysis.ts is for.
 */
export async function generateAnalysis(assessmentId: string): Promise<GenerateAnalysisResult> {
  ...
  // Caching: if an analysis already exists, return it instead of calling
  // the AI again.
  if (assessment.aiAnalysis) {
    return { success: true, analysisId: assessment.aiAnalysis.id };
  }

  // Group answers by section title with their question label, in the
  // fixed section order — this is what goes into the AI prompt.
  ...
  if (!aiResult.success || !aiResult.data) {
    return { success: false, error: aiResult.error ?? "Failed to generate analysis" };
  }
