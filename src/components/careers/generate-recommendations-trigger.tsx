...
import { generateRecommendations } from "@/actions/careers/generate-recommendations";
...
interface GenerateRecommendationsTriggerProps {
  analysisId: string;
}

/**
 * Rendered when an AIAnalysis exists but has no CareerRecommendations
 * yet. Same auto-trigger-on-mount pattern as GenerateAnalysisTrigger
 * from Task 8 Module 5 — generation happens as part of navigating to
 * the Careers page, not as a separate deliberate click.
 */
export function GenerateRecommendationsTrigger({ analysisId }: GenerateRecommendationsTriggerProps) {
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        setError(result.error ?? "Failed to generate career recommendations");
        return;
      }
      router.refresh();
    });
